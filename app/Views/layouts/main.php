...
<div id="app">
        ...
        <div class="topbar-actions">
            <?php
                ...
                if ($userId) {
                    ...
                    // Compute topbar notifications (bills, debts, taxes, broadcasts)
                    try {
                        ...
                        usort($_layoutNotifs, fn($a, $b) => ($a['days_left'] ?? 0) <=> ($b['days_left'] ?? 0));
                    } catch (\Throwable $e) {
                        $_layoutNotifs = [];
                    }
                } else {
    ...
    <!-- FLOATING ACTION BUTTON (+ Catat Transaksi Pengeluaran / Pemasukan) -->
    <button class="fab-floating-add" id="fabBtn" type="button" title="Catat Transaksi (Pemasukan / Pengeluaran)" aria-label="Catat Transaksi" onclick="if(window.openTransactionModal){window.openTransactionModal();}else if(window.openModal){window.openModal();}else{document.getElementById('txModalOverlay')?.classList.add('open');}">
...
<div class="modal-overlay" id="txModalOverlay" onclick="if(event.target===this){if(window.closeModal){window.closeModal();}else{this.classList.remove('open');}}">
    <div class="modal-sheet" id="txModal">
        ...
        <form id="txForm" autocomplete="off">
            ...
            <input type="hidden" id="txExistingImage" name="existing_image">

            <!-- Quick OCR Scan Shortcut -->
            <a href="/scan" class="ocr-quick-modal-btn" id="btnQuickOcr">
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" width="16" height="16">
                    <path d="M23 19a2 2 0 0 1-2 2H3a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h4l2-3h6l2 3h4a2 2 0 0 1 2 2z"/>
                    <circle cx="12" cy="13" r="4"/>
                </svg>
                <span>📷 Scan Struk / Nota (OCR) Otomatis</span>
            </a>
                ...
                <div class="notif-item-card <?= $cardClass ?>">
                    <div style="display:flex;align-items:flex-start;gap:10px;flex:1;min-width:0">
                        ...
                        <div style="min-width:0">
                            <div style="font-size:13.5px;font-weight:800;color:var(--text-primary);margin-bottom:2px"><?= esc($item['title'] ?? 'Pemberitahuan') ?></div>
                            <div style="font-size:11.5px;font-weight:700;color:<?= $badgeColor ?>">
                                <?= esc($item['subtitle'] ?? '') ?>
                            </div>
                            <?php if (!empty($item['amount']) && (float)$item['amount'] > 0): ?>
                            <div style="font-size:12px;font-weight:800;color:var(--text-primary);margin-top:2px">
                                <?= esc($symbol ?? 'Rp') ?> <?= number_format((float)$item['amount'], 0, ',', '.') ?>
                            </div>
                            <?php endif; ?>
                        </div>
                    </div>
