...
class DuitkuWidgetLarge : HomeWidgetProvider() {
    ...
    override fun onUpdate(
        ...
        for (appWidgetId in appWidgetIds) {
            try {
                ...
                Log.d(TAG, "Large widget [$appWidgetId]: balance=$balance, income=$income, expense=$expense, month=$month")
                ...
                Log.e(TAG, "Large widget [$appWidgetId] onUpdate failed", e)
            }
        }
    }
    ...
    private fun bindCategories(context: Context, views: RemoteViews, prefs: SharedPreferences) {
        try {
            ...
            // Colors stored as String by Dart side — read safely
            val catColors = listOf(
                safeColor(prefs, KEY_CAT_COLOR_1, 0xFF64748B.toInt()),
                safeColor(prefs, KEY_CAT_COLOR_2, 0xFF94A3B8.toInt()),
                safeColor(prefs, KEY_CAT_COLOR_3, 0xFFCBD5E1.toInt()),
            )
            ...
            Log.e(TAG, "bindCategories failed", e)
        }
    }

    /**
     * Safely read color from SharedPreferences.
     * Dart stores colors as int via home_widget which may serialize as Long.
     * We try getInt → getLong → getString fallback to avoid ClassCastException.
     */
    private fun safeColor(prefs: SharedPreferences, key: String, default: Int): Int {
        return try {
    ...
    companion object {
        private const val TAG = "DuitkuWidgetLarge"
