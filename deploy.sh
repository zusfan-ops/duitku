...
if [ -f "$PROJECT_DIR/$DB_CONFIG_FILE" ]; then
    if cp -f "$PROJECT_DIR/$DB_CONFIG_FILE" "$BACKUP_DIR/Database.php.server" 2>/dev/null; then
    ...
    if [ ! -f "$PROJECT_DIR/writable/Database.php.production" ]; then
        cp -f "$PROJECT_DIR/$DB_CONFIG_FILE" "$PROJECT_DIR/writable/Database.php.production" 2>/dev/null || true
    fi
fi

# Backup .env dan db_config.php jika ada
if [ -f "$PROJECT_DIR/.env" ]; then
    cp -f "$PROJECT_DIR/.env" "$BACKUP_DIR/.env.last" 2>/dev/null || true
fi
if [ -f "$PROJECT_DIR/app/Config/db_config.php" ]; then
    cp -f "$PROJECT_DIR/app/Config/db_config.php" "$BACKUP_DIR/db_config.php.last" 2>/dev/null || true
fi
...
if [ ! -f "$PROJECT_DIR/.env" ] && [ -f "$BACKUP_DIR/.env.last" ]; then
    cp -f "$BACKUP_DIR/.env.last" "$PROJECT_DIR/.env" 2>/dev/null || true
fi
if [ ! -f "$PROJECT_DIR/app/Config/db_config.php" ] && [ -f "$BACKUP_DIR/db_config.php.last" ]; then
    cp -f "$BACKUP_DIR/db_config.php.last" "$PROJECT_DIR/app/Config/db_config.php" 2>/dev/null || true
fi
